...
def add_salt_pepper_batch_tensor(x, severity=1):
    ...
    salt_or_pepper = torch.randint(
        0, 2, x.shape, device=x.device, dtype=torch.int64
    ).to(x.dtype)
...
def add_clouds_batch_tensor(x, severity=1):
    ...
    B, C, H, W = x.shape
    mask = torch.rand(B, 1, H, W, device=x.device, dtype=x.dtype)
    ...
    cloud_color = torch.full_like(x, 0.9, device=x.device)
...
def add_haze_batch_tensor(x, severity=1):
    ...
    haze_layer = torch.ones_like(x, device=x.device)
...
def create_data_gaps_batch_tensor(x, severity=1):
    ...
    B, C, H, W = x.shape
    angles = torch.rand(B, device=x.device, dtype=x.dtype) * np.pi
    processed = []
